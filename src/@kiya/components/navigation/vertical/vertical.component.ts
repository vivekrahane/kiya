...
import { KiyaNavigationItem, KiyaVerticalNavigationAppearance, KiyaVerticalNavigationMode, KiyaVerticalNavigationPosition } from '@kiya/components/navigation/navigation.types';
...
@Component({
    selector       : 'kiya-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : kiyaAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'kiyaVerticalNavigation'
})
export class KiyaVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: KiyaVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: KiyaVerticalNavigationMode = 'side';
    @Input() name: string = this._kiyaUtilsService.randomId();
    @Input() navigation: KiyaNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: KiyaVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<KiyaVerticalNavigationAppearance> = new EventEmitter<KiyaVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<KiyaVerticalNavigationMode> = new EventEmitter<KiyaVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<KiyaVerticalNavigationPosition> = new EventEmitter<KiyaVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<KiyaNavigationItem> = new ReplaySubject<KiyaNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<KiyaNavigationItem> = new ReplaySubject<KiyaNavigationItem>(1);
    ...
    private _kiyaScrollbarDirectives!: QueryList<KiyaScrollbarDirective>;
    private _kiyaScrollbarDirectivesSubscription: Subscription;
    ...
    {
        return {
            'kiya-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`kiya-vertical-navigation-appearance-${this.appearance}`]: true,
            'kiya-vertical-navigation-hover'                          : this._hovered,
            'kiya-vertical-navigation-inner'                          : this.inner,
            'kiya-vertical-navigation-mode-over'                      : this.mode === 'over',
            'kiya-vertical-navigation-mode-side'                      : this.mode === 'side',
            'kiya-vertical-navigation-opened'                         : this.opened,
            'kiya-vertical-navigation-position-left'                  : this.position === 'left',
            'kiya-vertical-navigation-position-right'                 : this.position === 'right'
        };
    }
    ...
    /**
     * Setter for kiyaScrollbarDirectives
     */
    @ViewChildren(KiyaScrollbarDirective)
    set kiyaScrollbarDirectives(kiyaScrollbarDirectives: QueryList<KiyaScrollbarDirective>)
    {
        // Store the directives
        this._kiyaScrollbarDirectives = kiyaScrollbarDirectives;

        // Return if there are no directives
        if ( kiyaScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._kiyaScrollbarDirectivesSubscription )
        {
            this._kiyaScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._kiyaScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    kiyaScrollbarDirectives.forEach((kiyaScrollbarDirective) => {
                        kiyaScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void
    {
        setTimeout(() => {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.kiya-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._kiyaScrollbarDirectives.forEach((kiyaScrollbarDirective) => {

                    // Skip if not enabled
                    if ( !kiyaScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    kiyaScrollbarDirective.scrollToElement('.kiya-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: KiyaNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: KiyaNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('kiya-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('kiya-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('kiya-vertical-navigation-aside-overlay');
