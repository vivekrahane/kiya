...
import { KiyaMockApiService } from '@kiya/lib/mock-api/mock-api.service';

@Injectable({
    providedIn: 'root'
})
export class KiyaMockApiInterceptor implements HttpInterceptor
{
    /**
     * Constructor
     */
    constructor(
        @Inject(KIYA_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _kiyaMockApiService: KiyaMockApiService
    )
    {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>
    {
        ...
        const {
                  handler,
                  urlParams
              } = this._kiyaMockApiService.findHandler(request.method.toUpperCase(), request.url);
